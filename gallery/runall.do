...
display as text _n "{hline 70}"
display as text "7a3. the response braid: answers flowing item to item"
display as text "{hline 70}"
* the flow map above annotates a routing spine; this one follows the answers
* themselves: a column per item, a block per common answer, ribbons carrying
* the respondents between them, and the most common complete paths tabled
use fake_survey.dta, clear
...
file write `gh' `"<p class="sub">Running <code>gallery/runall.do</code> regenerates every file here from one fake survey, so each page shows the command&#8217;s own output.</p>"' _n

file write `gh' "<h2>The flow map</h2>" _n
file write `gh' "<p>The map draws a twelve-item poll, weighted and branched on party and on whether the respondent voted. Dashed grey boxes are questions that lane was never shown.</p>" _n
...
file write `gh' "<p>The flow map above follows the routing; this view follows the answers. Each item becomes a column, each common answer becomes a block, and the ribbons carry respondents from answer to answer, with the most common complete paths tabled underneath. Each page opens with a step-by-step reading guide written in its own item names.</p>" _n
...
file write `gh' "<p>These pages draw the spine map with each item&#8217;s common answers inside its box, and the same map cut to one subgroup with the scope stated on the page.</p>" _n
file write `gh' `"<p><a href="g_responses.html">g_responses.html</a> &#183; <a href="g_scoped.html">g_scoped.html</a> &#183; <a href="g_profile.html">g_profile.html</a> (lanes by respondent behaviour)</p>"' _n

file write `gh' "<h2>The same map as a figure</h2>" _n
file write `gh' "<p>Stata&#8217;s own graph engine draws it, for a paper or a slide.</p>" _n
...
file write `gh' "<h2>As mermaid text</h2>" _n
file write `gh' "<p>GitHub and Quarto render the diagram from the text.</p>" _n
file write `gh' `"<p><a href="g_map.mmd">g_map.mmd</a> &#183; <a href="g_map.md">g_map.md</a></p>"' _n

file write `gh' "<h2>The journals</h2>" _n
file write `gh' "<p>Each journal has one tab-separated line per event. Every picture above is built from one of these, so a drawing can be re-cut without reading the data again.</p>" _n
